...
// Star icon for testimonials (unchanged)
const StarIcon = () => (
    <svg className="h-5 w-5 text-amber-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
);

const BrainBgIcon = ({ isVisible }) => (
  <svg
...
function WhyChooseUs() {
    ...
    const testimonials = [
        ...
        { quote: "I wasn’t the biggest fitness guy, but thanks to this I finally found an easy way to help my drive and focus! The product is a motivator of its own.", name: "Korey Torres" },
        { quote: "Highly recommended! Bold X was a game changer for me; don't think its just for the guys!", name: "Katy Gilmore" },
        { quote: "The ingredients are really top-notch, which is amazing considering what other products deliver for the same price. This is a bold product; no pun intended!", name: "Terry Dennis" },
    ];

    return (
        <div className="bg-white dark:bg-brand-dark">
            <header className={`bg-blue-50 py-16 px-4 text-center transition-all duration-700 ease-out dark:bg-gray-800 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
                <h1 className="text-5xl font-bold text-gray-800 dark:text-brand-accent sm:text-5xl">The Bold Difference</h1>
                <p className="mx-auto mt-5 max-w-4xl text-lg text-gray-600 dark:text-gray-300">
                    We use only the highest quality, non-GMO herbs and botanicals. No artificial fillers, ever. Our blends are crafted by experts and backed by research to ensure effectiveness and safety. 
                    <br/><br/>Discover why our commitment to quality, purity, and performance sets us apart.
                </p>
            </header>

            <section
                className="relative overflow-hidden bg-lime-100 py-20 px-4 dark:bg-orange-800 sm:px-6 lg:px-8"
                onMouseLeave={() => setHoveredFeature(null)}
                >
                {/* Background Icon Layer */}
                <div className="absolute inset-0 z-0">
                    <BrainBgIcon isVisible={hoveredFeature === 'overlay-mental'} />
                    <BeakerBgIcon isVisible={hoveredFeature === 'overlay-beakers'} />
                    <SpeedBgIcon isVisible={hoveredFeature === 'overlay-speed'} />
                </div>

                {/* Features (above icons) */}
                <div className="relative z-10 mx-auto max-w-4xl space-y-16">
                    {features.map((feature, index) => (
                    <div
                        key={feature.number}
                        className={`group relative w-full transform rounded-lg bg-blue-50 p-6 shadow-xl transition-all duration-500 ease-out hover:scale-[1.01] hover:shadow-2xl dark:bg-gray-900 md:w-3/4 
                        ${feature.indentClass} 
                        ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
                        style={{ transitionDelay: `${index * 200}ms` }}
                        onMouseEnter={() => setHoveredFeature(feature.overlayClass)}
                    >
                        <div className="relative z-10 pl-4">
                        <h3 className="font-sora text-3xl font-bold text-blue-600 dark:text-brand-accent">
                            {feature.title}
                        </h3>
                        ...
                        <span className="absolute -top-12 -left-20 z-0 font-sora text-8xl font-bold text-orange-300 dark:text-gray-300">
                        {feature.number}
                        </span>
                    </div>
                    ))}
            ...
            <section className="bg-blue-50 py-20 px-4 dark:bg-gray-800 sm:px-6 lg:px-8">
                <div className="mx-auto max-w-5xl">
                    <h2 className="text-center text-3xl font-bold">What Our Community Says</h2>
                    <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-3">
                        {testimonials.map((testimonial, index) => (
                             <div key={index} className={`flex flex-col rounded-lg bg-white p-8 shadow-lg transition-all duration-500 ease-out dark:bg-brand-dark ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'}`} style={{ transitionDelay: `${300 + index * 150}ms` }}>
                                <div className="flex">
                                    {[...Array(5)].map((_, i) => <StarIcon key={i} />)}
                                </div>
                                <blockquote className="mt-4 flex-grow text-gray-600 dark:text-gray-300">"{testimonial.quote}"</blockquote>
                                <footer className="mt-4 font-sora font-semibold">- {testimonial.name}</footer>
                            </div>
                        ))}
                    </div>
                </div>
